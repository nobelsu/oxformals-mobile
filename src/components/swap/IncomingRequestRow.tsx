...
export function IncomingRequestRow({
  ...
  }, [getOrCreateConversation, router, fromUser.id]);
  ...
  return (
    <SketchCard seed={seedFrom(request.id)} padding={16}>
      <Pressable
        ...
        accessibilityLabel={
          formalsA11y
            ? `View request from ${fromUser.name}, ${formalsA11y}`
            : `View request from ${fromUser.name}`
        }
      >
        <View style={styles.headerRow}>
          <View style={styles.avatarCol}>
            <Avatar avatar={fromUser.avatar} name={fromUser.name} size={48} />
          </View>
